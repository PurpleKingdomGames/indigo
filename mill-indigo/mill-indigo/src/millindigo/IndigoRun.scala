...
object IndigoRun {
  ...
  def filesToWrite(windowWidth: Int, windowHeight: Int): List[FileToWrite] =
    List(
      FileToWrite("main.js", mainFile(windowWidth, windowHeight)),
      FileToWrite("preload.js", preloadFile),
      FileToWrite("package.json", packageFile)
    )

  def mainFile(windowWidth: Int, windowHeight: Int): String =
    s"""
