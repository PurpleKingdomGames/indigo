...
import com.purplekingdomgames.indigoexts.pathfinding.GridSquare.{EmptySquare, EndSquare, ImpassableSquare, StartSquare}
...
object SearchGrid {
  ...
  def scoreGridSquares(searchGrid: SearchGrid): List[GridSquare] = {
    @tailrec
    def rec(target: Coords,
            unscored: List[GridSquare],
            scoreValue: Int,
            lastCoords: List[Coords],
            scored: List[GridSquare]): List[GridSquare] =
  ...
  def locatePath(searchGrid: SearchGrid): List[Coords] = {
    def rec(currentPosition: Coords,
            currentScore: Int,
            target: Coords,
            grid: SearchGrid,
            width: Int,
            acc: List[Coords]): List[Coords] =
