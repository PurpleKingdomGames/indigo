...
class SceneManager[GameModel, ViewModel](scenes: ScenesList[GameModel, ViewModel, _, _], scenesFinder: SceneFinder) {
  ...
  def updateModel(gameTime: GameTime, model: GameModel): GameEvent => GameModel = {
    ...
    case event =>
      scenes.findScene(finderInstance.current.name) match {
        case None =>
          Logger.errorOnce("Could not find scene called: " + finderInstance.current.name.name)
          model
  ...
  def updateView(gameTime: GameTime,
                 model: GameModel,
                 viewModel: ViewModel,
...
object SceneManager {
  ...
  def apply[GameModel, ViewModel](scenes: ScenesList[GameModel, ViewModel, _, _],
                                  initialScene: SceneName): SceneManager[GameModel, ViewModel] =
    new SceneManager[GameModel, ViewModel](scenes,
                                           SceneFinder.fromScenes[GameModel, ViewModel](scenes).jumpToSceneByName(initialScene))
