  ...
  given [ReferenceData]: Component[Button[ReferenceData], ReferenceData] with
    ...
    def updateModel(
      ...
      case FrameTick =>
        ...
        val decideState: ButtonState =
          if context.isActive || model.isDragged then
            ...
            then
              if context.frame.input.pointer.isLeftDown then ButtonState.Down
      ...
      case _: PointerEvent.Move
          if (context.isActive || model.isDragged) && model.isDown && !context.frame.input.pointer.isLeftDown =>
