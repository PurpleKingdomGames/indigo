...
object DefaultPathBuilders:
  ...
  def fromImpassablePoints(
    ...
    val neighboursFilter = (p: Point) => p.x >= 0 && p.x < width && p.y >= 0 && p.y < height && !impassablePoints.contains(p)
  ...
  /** Builds a path finder builder from a weighted 2D grid.
   * Impassable points are represented by Int.MaxValue
   * other points are represented by their weight/ cost
   * grid(y)(x) is the weight of the point (x, y)
   *
   * @param grid
  ...
  def fromWeighted2DGrid(
    ...
    val neighboursFilter = (p: Point) => p.x >= 0 && p.x < width && p.y >= 0 && p.y < height && grid(p.y)(p.x) != Int.MaxValue
    val buildNeighbours  = buildPointNeighbours(allowedMovements, neighboursFilter)
  ...
  /** Builds a path finder builder from a weighted 1D grid.
   * Impassable points are represented by Int.MaxValue
   * other points are represented by their weight/ cost
   * grid(y * width + x) is the weight of the point (x, y)
   *
   * @param grid
  ...
  def fromWeightedGrid(
    ...
    val neighboursFilter = (p: Point) => p.x >= 0 && p.x < width && p.y >= 0 && p.y < height && grid(p.y * width + p.x) != Int.MaxValue
    val buildNeighbours  = buildPointNeighbours(allowedMovements, neighboursFilter)
