...
object TestContext {
  ...
  @tailrec
  private def buildPath(width: Int, height: Int, allowedMoves: List[Point], dice: Dice, path: List[Point], currentPosition: Point): List[Point] =
  ...
  private def computeNextPosition(width: Int, height: Int, path: List[Point], allowedMoves: List[Point], currentPosition: Point): Option[Point] =
    allowedMoves
...
final class PathFinderTests extends Properties("PathFinder") {
  ...
  property("return Some(list(start)) when start and end are the same") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromAllowedPoints(context.allowedPoints.toSet, context.allowedMoves)
      PathFinder.findPath(start, start, pathBuilder) == Some(List(start))
    }

  property("return None when start and end are not connected") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      ...
      if(start != end && !context.allowedMoves.map(_ + start).contains(end))
        ...
        val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromAllowedPoints(newContext.allowedPoints.toSet, newContext.allowedMoves)
        PathFinder.findPath(start, end, pathBuilder) == None
      else true
    }

  property("return a path with a length <= to the generated one") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
  ...
  property("return a path that is a subset of the generated one") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromAllowedPoints(context.allowedPoints.toSet, context.allowedMoves)
  ...
  property("return a path with a length > 1 when start and end are connected") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      if(start != end)
  ...
  property("not have duplicated entries") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromAllowedPoints(context.allowedPoints.toSet, context.allowedMoves)
  ...
  property("return a list of adjacent entries") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromAllowedPoints(context.allowedPoints.toSet, context.allowedMoves)
  ...
  property("build a path from the impassable points") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromImpassablePoints(context.impassablePoints.toSet, context.width, context.height, context.allowedMoves)
      PathFinder.findPath(start, end, pathBuilder)
        .fold(false)(path => path.head == start && path.last == end && path.length >= 1 && path.length <= context.path.length)
    }

  property("build a path from a weighted 2D grid") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val pathBuilder: PathBuilder[Point] = DefaultPathBuilders.fromWeighted2DGrid(context.weighted2DGrid, context.width, context.height, context.allowedMoves)
      PathFinder.findPath(start, end, pathBuilder)
        .fold(false)(path => path.head == start && path.last == end && path.length >= 1 && path.length <= context.path.length)
    }

  property("produce the same result with weighted 1D grid and 2D grid") =
    Prop.forAll(genContext) { context =>
      val start: Point = context.dice.shuffle(context.allowedPoints).head
      val end: Point = context.dice.shuffle(context.allowedPoints).head
      val a2DGrid = context.weighted2DGrid
      val pathBuilder1: PathBuilder[Point] = DefaultPathBuilders.fromWeighted2DGrid(a2DGrid, context.width, context.height, context.allowedMoves)
      val pathBuilder2: PathBuilder[Point] = DefaultPathBuilders.fromWeightedGrid(a2DGrid.flatten.toJSArray, context.width, context.height, context.allowedMoves)
      PathFinder.findPath(start, end, pathBuilder1) == PathFinder.findPath(start, end, pathBuilder2)
    }

  property("properly handles weighted grid") =
    Prop.forAll(weightedGrid) { (width, height, dice, grid) =>
  ...
  property("allow to find a path using a custom type") =
    Prop.forAll(genContext) { context =>
      ...
      val pathBuilder: PathBuilder[PointWithUserContext] =
        new PathBuilder[PointWithUserContext]:
          def neighbours(t: PointWithUserContext): List[PointWithUserContext] =
              context.allowedMoves.map(_ + t.point)
          ...
          def distance(t1: PointWithUserContext, t2: PointWithUserContext): Int =
            if (t1.point.x == t2.point.x || t1.point.y == t2.point.y) DefaultPathBuilders.DefaultSideCost else DefaultPathBuilders.DefaultDiagonalCost

          def heuristic(t1: PointWithUserContext, t2: PointWithUserContext): Int =
            (Math.abs(t1.point.x - t2.point.x) + Math.abs(t1.point.y - t2.point.y)) * DefaultPathBuilders.DefaultMaxHeuristicFactor

      PathFinder.findPath(start, end, pathBuilder).fold(false)(path => path.head == start && path.last == end && path.length >= 1 && path.length <= context.path.length)
    }
