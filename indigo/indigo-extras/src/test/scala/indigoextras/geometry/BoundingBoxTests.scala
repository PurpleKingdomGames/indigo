...
class BoundingBoxTests extends munit.FunSuite {
      ...
      test("creating rectangles") {
        ...
        test("should be able to construct a bounding box from two vertices") {
          val pt1 = Vertex(5, 6)
          val pt2 = Vertex(1, 3)
        ...
        test("should be able to construct a bounding box from a cloud of vertices") {
        ...
        test("should be able to construct a bounding box from a rectangle") {
      ...
      test("Expand to include two bounding boxes") {

        test("should return the original bounding box when it already encompasses the second one") {
        ...
        test("should expand to meet the bounds of both") {
      ...
      test("intersecting vertices") {

        test("should be able to detect if the point is inside the BoundingBox") {
          assertEquals(BoundingBox(0, 0, 10, 10).isVertexWithin(Vertex(5, 5)), true)
        }

        test("should be able to detect that a point is outside the BoundingBox") {
      ...
      test("intersecting lines") {
        ...
        test("should find the intersection for the line passing through the bounding box") {
          val actual =
            BoundingBox(1, 1, 3, 3)
              .lineIntersectsAt(
        ...
        test("should not find intersection for a line outside the bounding box") {
          BoundingBox(5, 5, 4, 4)
            assertEquals(.lineIntersectsAt(LineSegment((2d, 0d), (4d, 4d))), None)
        }

      }

      test("encompasing rectangles") {
        test("should return true when A encompases B") {
        ...
        test("should return false when A does not encompass B") {
        ...
        test("should return false when A and B merely intersect") {
      ...
      test("overlapping bounding boxes") {
        test("should return true when A overlaps B") {
          val a = BoundingBox(10, 10, 20, 20)
        ...
        test("should return false when A and B do not overlap") {
          val a = BoundingBox(10, 10, 20, 20)
          val b = BoundingBox(100, 100, 100, 100)
      ...
      test("Expand") {
        test("should be able to expand in size by a given amount") {
          val a = BoundingBox(10, 10, 20, 20)
