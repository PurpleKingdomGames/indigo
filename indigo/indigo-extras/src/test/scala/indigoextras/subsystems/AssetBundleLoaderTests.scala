  ...
  val defaultAssets: List[AssetTypePrimitive] =
    List(
      ...
      test("AssetBundleLoader - Journey (happy path)") {
        ...
        assertEquals(defaultAssets.length + 1, loadOutcome.globalEvents.length)
        defaultAssets.forall { asset =>
          loadOutcome.globalEvents.contains(AssetEvent.LoadAsset(asset, BindingKey(asset.path.value), false))
        assertEquals(}, true)
        assertEquals(loadOutcome.globalEvents.contains(AssetBundleLoaderEvent.Started(key)), true)
        ...
        assertEquals(nextLoader1.globalEvents.length, 1)
        assertEquals(nextLoader1.globalEvents.contains(AssetBundleLoaderEvent.LoadProgress(key, 33, 1, 3)), true)

        val nextLoader2 = loader.update(context(0), nextLoader1.state)(AssetEvent.AssetBatchLoaded(BindingKey("/image_2.png"), false))
        assertEquals(nextLoader2.globalEvents.length, 1)
        assertEquals(nextLoader2.globalEvents.contains(AssetBundleLoaderEvent.LoadProgress(key, 67, 2, 3)), true)
        ...
        assertEquals(nextLoader3.globalEvents.length, 2)
        assertEquals(nextLoader3.globalEvents.contains(AssetBundleLoaderEvent.LoadProgress(key, 100, 3, 3)), true)
        assertEquals(nextLoader3.globalEvents.contains(AssetEvent.LoadAssetBatch(defaultAssets.toSet, key, true)), true)
        ...
        assertEquals(finalLoader.globalEvents.length, 1)
        assertEquals(finalLoader.globalEvents.contains(AssetBundleLoaderEvent.Success(key)), true)
      ...
      test("AssetBundleLoader - Journey (unhappy path)") {
        ...
        assertEquals(nextLoader3.globalEvents.length, 2)
        assertEquals(nextLoader3.globalEvents.contains(AssetBundleLoaderEvent.LoadProgress(key, 100, 3, 3)), true)
        assertEquals(nextLoader3.globalEvents.contains(AssetEvent.AssetBatchLoadError(key, "Asset batch with key 'test' failed to load")), true)
        ...
        assertEquals(finalLoader.globalEvents.length, 1)
        assertEquals(finalLoader.globalEvents.contains(AssetBundleLoaderEvent.Failure(key, "Asset batch with key 'test' failed to load")), true)
      ...
      test("AssetBundleTracker") {

        test("Can add a bundle to the tracker") {
        ...
        test("Doesn't add empty bundles") {
          assertEquals(AssetBundleTracker.empty.bundleCount, 0)
          assertEquals(AssetBundleTracker.empty.addBundle(BindingKey("a"), Nil).bundleCount, 0)
        }

        test("Doesn't re-add or replace bundles with existing identical keys") {
        ...
        test("Can get the asset count for a bundle in the tracker") {
        ...
        test("Initially, all assets are not completed or loaded") {
          val tracker = AssetBundleTracker.empty.addBundle(BindingKey("a"), defaultAssets)
          assertEquals(tracker.findBundleByKey(BindingKey("a")).get.assets.toList.forall(p => !p._2.complete && !p._2.loaded), true)
        }

        test("Can update an asset in a bundle with a completed + loaded state (success)") {
          val tracker =
        ...
        test("Can update an asset in a bundle with a completed + loaded state (failure)") {
          val tracker =
            AssetBundleTracker.empty
              .addBundle(BindingKey("bundle"), defaultAssets)
        ...
        test("Bundle status: Can check if all assets in a bundle are completed (success - 100%)") {
          val tracker =
            AssetBundleTracker.empty
        ...
        test("Bundle status: Can check if all assets in a bundle are completed (failed - ?%)") {
          val tracker =
            AssetBundleTracker.empty
              .addBundle(BindingKey("a"), defaultAssets)
              .assetLoadComplete(AssetPath("/image_1.png"), true)
        ...
        test("Bundle status: Can check if a load is incomplete with percent done") {
          val tracker =
            AssetBundleTracker.empty
              .addBundle(BindingKey("a"), defaultAssets)
              .assetLoadComplete(AssetPath("/image_1.png"), true)
        ...
        test("Assets are not unique to a bundle, all instances are set as loaded") {
          val lookup =
            AssetBundleTracker.empty
              .addBundle(BindingKey("a"), defaultAssets)
        ...
        test("Can look up the loaded state of all instances of an asset") {
          val lookup1 =
            AssetBundleTracker.empty
              .addBundle(BindingKey("a"), defaultAssets)
              .findAssetByPath(AssetPath("/image_1.png"))
        ...
        test("Does the tracker contain a bundle?") {
          val tracker = AssetBundleTracker.empty
            .addBundle(BindingKey("a"), defaultAssets)
            .addBundle(BindingKey("b"), defaultAssets)
        ...
        test("Does the tracker contain an asset in any bundle?") {
          val tracker = AssetBundleTracker.empty
            .addBundle(BindingKey("a"), defaultAssets)
