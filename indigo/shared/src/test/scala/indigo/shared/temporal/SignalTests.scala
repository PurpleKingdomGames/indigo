...
class SignalTests extends munit.FunSuite {
      ...
      test("Signals") {
        test("should be able to get a fixed value from a signal") {
          assertEquals(Signal.fixed("a").at(Seconds(100)), "a")
        }

        test("should be able to get a value over time from a signal") {
        ...
        test("should be able to merge signals") {
        ...
        test("should be able to flatMap Signals") {
        ...
        test("should be able to flatMap Signals in a for comp") {
...
Where a thing moves in a circle for 2 seconds and then stops.
      ...
      test("Utils") {
        ...
        test("SmoothPulse smoothly interpolates from 0 to 1") {
          assertEquals(round(Signal.SmoothPulse.at(Seconds(0))), 0)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(0.25))), 0.5)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(0.5))), 1)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(0.75))), 0.5)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(1))), 0)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(1.5))), 1)
          assertEquals(round(Signal.SmoothPulse.at(Seconds(2))), 0)
        }

        test("Pulse produces a signal of true and false values") {
        ...
        test("clamping time limits the time so that any time passed can be valid") {
        ...
        test("wrapping time keeps time looping round a fixed point") {
