...
class RectangleTests extends munit.FunSuite {
      ...
      test("Expand to include two rectangles") {

        test("should return the original rectangle when it already encompasses the second one") {
      ...
      test("intersecting points") {

        test("should be able to detect if the point is inside the Rectangle") {
          assertEquals(Rectangle(0, 0, 10, 10).isPointWithin(Point(5, 5)), true)
        }

        test("should be able to detect that a point is outside the Rectangle") {
