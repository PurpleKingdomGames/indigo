...
object Animation {
  ...
  implicit def animationAsString(
    ...
    AsString.create { a =>
      s"Animation(${sAK.show(a.animationKey)}, imageAssetRef, ${sP.show(a.spriteSheetSize)}, ${sCL.show(a.currentCycleLabel)}, ${sNelC.show(a.cycles)}, ${sLA.show(a.actions)})"
    }

  def create(animationKey: AnimationKey, imageAssetRef: String, spriteSheetSize: Point, cycle: Cycle): Animation =
    apply(animationKey, imageAssetRef, spriteSheetSize, cycle.label, NonEmptyList(cycle), Nil)
